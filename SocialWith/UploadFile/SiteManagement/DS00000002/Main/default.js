$(function () {
    ...
    $('.month').text((new Date().getMonth() + 1) + "월");

    $('#quickClose').on("click", function (e) {
        e.preventDefault();
        $('#quick').hide();
    });

    fnSetSalesHistoryList();
...
function fnGetDistRoll(response) {
    if (!isEmpty(response)) {
        ...
        $.each(response, function (key, value) { //테이블 추가
            ...
            createhtml += ' <li ' + liClass + ' id="LiMasterRoll' + value.SEQ + '" idx="' + index + '"><a href="' + redirectUrl + '">' + value.ROLLNAME + '</a><input type="hidden" id="hdRollImgPath" value="' + rollimgPath + '"/><input type="hidden" id="hdRollImgUrl" value="' + value.ROLLBANNERURL + '"/><input type="hidden" id="hdRollUrl" value="' + hdRollUrlVal + '"></li>'
...
function fnSetSalesHistoryList() {
    var asynList = "";
    var compName = (userGubun == 'BU0002' ? "가스공사" : "");
   

    //모바일/PC 여부 검사해서 월 구매내역 팝업 Display 설정
    ...
    var callback = function (response) {
        ...
        if (!isEmpty(response)) {
            ...
            $.each(response, function (index, value) {
                var url = "Goods/GoodsShoppingList";

                asynList += "<li class='li' onclick='fnFormClick(\"" + value.Company_Code + "\");' style='cursor: pointer' >"
                asynList += "<form name='" + value.Company_Code + "' method='POST' action='"+ url +"' target='_self'  >"
                asynList += "<input type='hidden' name='CompCode' value='" + value.Company_Code +"' /> "
                asynList += "<input type='hidden' name='FullComp' value='" + value.Full_company_name +"' /> </form>"
                asynList += "<input type='hidden' id='hdCpCode' data-code='" + value.Company_Code + "' val='" + value.Company_Code + "' />"
                asynList += "<span class='list_name' >" + value.Company_Name + "</span>"
                asynList += "<span class='price'>" + numberWithCommas(value.GoodsSalePriceVat) + "원</span></a>"
                asynList += "</li>"

             
            });
        }
        $("#quickMenuList").append(asynList);
