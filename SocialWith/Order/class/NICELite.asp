...
Class clssNICEpay
	...
	Public Function startAction()
		...
		'결제 시에 TID 생성
		'edit by xoghzone_2018_08_23 ARS 결제 추가 
		'IF(m_status) AND (m_type = "securepay" OR m_type = "receipt" OR m_type = "repay") THEN
		IF(m_status) AND (m_type = "securepay" OR m_type = "receipt" OR m_type = "repay" OR m_type = "CARD_ARS") THEN
			'TID 생성 생성
			make_tid()
		...
		IF(m_status) THEN
			...
			'edit by xoghzone_2018_08_23 ARS 결제 추가 
			IF m_type = "CARD_ARS" THEN
				Field_Value = LiteRequest("cardArsProcess.jsp", m_send_data)
			END If
	...
	Public sub make_tid
		...
		'edit by xoghzone_2018_08_23 나이스페이먼츠 tid 생성은 30
		'if 40 = Len(m_tid) then
		if 30 = Len(m_tid) then
			m_status = 1
	...
	Public Function make_msg ()
		...
		IF m_type = "CARD_ARS" THEN
			...
			m_buyerauthnum = Request.Form("BuyerAuthNum")
			m_cardpwd = Request.Form("CardPwd")
			m_arsReqType = Request.Form("ArsReqType")
			...
			m_send_data = "TransType="&m_transtype&"&GoodsName="&m_goodname&"&Amt="&m_amt&"&Moid="&m_oid _ 
							...
							&"&EncodeKey="&Server.URLEncode(m_encodekey)&"&CardExpire="&m_cardexpire&"&CardNo="&m_cardno _ 
							&"&CardInterest="&m_cardinterest&"&AuthFlg="&m_auth_flg&"&CardQuota="&m_cardquota&"&BuyerAuthNum="&m_buyerauthnum&"&CardPwd="&m_cardpwd _
							&"&TrKey="&m_trkey&"&TID="&m_tid&"&CharSet="&m_charset&"&ArsReqType="&m_arsReqType
							
			'm_log.write_log m_send_data
